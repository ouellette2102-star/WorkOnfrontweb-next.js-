...
export function MessageInput({ onSend, disabled }: MessageInputProps) {
  ...
  return (
    <div className="border-t border-workon-border bg-white/80 p-4">
      {/* Error message */}
      {error && (
        ...
        <textarea
          ...
          className={`flex-1 resize-none rounded-2xl border px-4 py-3 text-workon-ink placeholder:text-workon-muted focus:outline-none disabled:opacity-50 ${
            error
              ...
              : "border-workon-border bg-white focus:border-workon-primary"
          }`}
          rows={2}
        />
        <Button
          ...
          className="self-end rounded-2xl bg-workon-primary px-6 py-3 font-semibold text-white transition hover:bg-workon-primary-hover disabled:opacity-50"
        >
          {isSending ? (
