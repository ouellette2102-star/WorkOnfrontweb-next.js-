          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-0.5">
              {Array.from({ length: 5 }, (_, i) => (
                <Star
                  key={i}
                  className={cn("h-4 w-4", i < Math.round(worker.averageRating)
            ...
            {hasReviews ? (
              <span className="text-sm font-semibold text-workon-ink">{worker.averageRating.toFixed(1)}</span>
            ) : null}
          ...
          {(worker.badges ?? []).length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {worker.badges.map((b) => (
                <span
                  key={b.type}
                  className="inline-flex items-center gap-1 rounded-full border border-workon-border bg-workon-bg px-2.5 py-1 text-[11px] font-medium text-workon-ink"
                >
                  {b.label}
                </span>
              ))}
