  ...
  const getStatusBadge = (status: MissionStatus) => {
    switch (status) {
      ...
      case MissionStatus.IN_PROGRESS:
        return (
          <span className="rounded-full bg-[#134021]/10 px-3 py-1 text-xs font-semibold text-[#134021]">
            🔄 En cours
          </span>
        );
      default:
      ...
      <div className="bg-white border border-[#EAE6DF] rounded-3xl p-5 shadow-card">
        ...
        <div className="space-y-4">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="h-32 animate-pulse rounded-2xl border border-[#EAE6DF] bg-[#F9F8F5]"
            />
          ))}
        </div>
      </div>
    ...
    return (
      <div className="bg-white border border-[#EAE6DF] rounded-3xl p-5 shadow-card">
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="mb-2 text-lg font-semibold text-[#1B1A18]">
            Aucune mission active
          </p>
          <p className="text-[#706E6A]">
            Commence par réserver une mission disponible
          </p>
          <Link href="/worker/missions">
            <Button className="mt-4 rounded-xl bg-[#134021] px-6 py-2 font-semibold text-white hover:bg-[#0F3319]">
              Voir les missions
            </Button>
          </Link>
        </div>
  ...
  return (
    <div className="bg-white border border-[#EAE6DF] rounded-3xl p-5 shadow-card">
      <div className="mb-6 flex items-center justify-between">
        ...
        <span className="text-[10px] text-[#706E6A] uppercase tracking-wider">
      ...
      <div className="space-y-4">
        {missions.map((mission) => (
          <div
            key={mission.id}
            className="group overflow-hidden bg-white border border-[#EAE6DF] rounded-2xl transition hover:shadow-soft"
          >
            <div className="px-4 py-3.5">
              {/* Header */}
              <div className="mb-4 flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="mb-1 text-xl font-bold text-[#1B1A18]">
                    {mission.title}
                  </h3>
                  {mission.city && (
                    <p className="text-sm text-[#706E6A]">📍 {mission.city}</p>
                  )}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {mission.status === MissionStatus.RESERVED && (
                  <Button className="bg-[#134021] text-white hover:bg-[#0F3319]">
