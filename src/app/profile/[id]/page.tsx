 ...
 * - Premium: badge, points forts (basé sur signaux réels)
...
import { Star, MessageCircle, Briefcase, MapPin, Calendar, CheckCircle2, Trophy } from "lucide-react";
import type {
  UserWithFullProfile,
  PortfolioItem,
  WorkerSkill,
...
import { isPremiumUser, getPremiumTier, getPremiumFeatures } from "@/lib/premium";
import { PremiumBadge } from "@/components/premium/premium-badge";
import { PremiumUpgradeCta } from "@/components/premium/premium-upgrade-cta";
...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  // Premium status (PR-26)
  const isPremium = isPremiumUser(workerProfile);
  const premiumTier = getPremiumTier(workerProfile);
  const premiumFeatures = getPremiumFeatures(premiumTier);

  // Points forts basés sur signaux réels (pour Premium)
  const highlights: string[] = [];
  if (completedMissions >= 10) highlights.push(`${completedMissions} missions réalisées`);
  if (averageRating >= 4.5 && reviewCount >= 3) highlights.push(`Note ${averageRating.toFixed(1)}/5`);
  if (skills.filter((s) => s.verified).length > 0) highlights.push("Compétences vérifiées");
  if (memberSince) highlights.push(`Membre depuis ${memberSince}`);
    ...
    <div className="min-h-screen bg-neutral-900 text-white">
      ...
      <ProfileHeader
        user={{
          ...
          badges: [
            ...(skills.filter((s) => s.verified).length > 0 ? ["Vérifié"] : []),
            ...(isPremium ? ["Premium"] : []),
          ],
          completedMissions,
          bio,
      ...
      <div className="pt-32 pb-28">
        <div className="max-w-2xl mx-auto px-4 space-y-4">
          ...
          {/* Quick Stats */}
          <Card className={`border-white/10 ${premiumFeatures.highlightedProfile ? "bg-gradient-to-br from-amber-500/5 to-neutral-800/50 border-amber-500/20" : "bg-neutral-800/50"}`}>
          ...
          {/* Points forts (Premium only, based on real signals) */}
          {premiumFeatures.highlightedProfile && highlights.length > 0 && (
            <Card className="border-amber-500/20 bg-gradient-to-r from-amber-500/10 to-yellow-500/10">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  <Trophy className="h-5 w-5 text-amber-400" />
                  <h3 className="font-semibold text-amber-400">Points forts</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {highlights.map((highlight, index) => (
                    <Badge
                      key={index}
                      variant="outline"
                      className="bg-amber-500/10 border-amber-500/30 text-white/80 text-xs"
                    >
                      {highlight}
                    </Badge>
                  ))}
          ...
          {/* Premium Upgrade CTA (for non-premium users viewing their own profile) */}
          {!isPremium && (
