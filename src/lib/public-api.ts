...
/** Public missions feed — always fresh after mission creation. */
...
export interface GetProsListParams {
  page?: number;
  pageSize?: number;
  city?: string;
  category?: string;
...
/** Browsable pros list — ISR 120s. Powers /pros UI. */
export const getProsList = (params: GetProsListParams = {}) => {
  const q = new URLSearchParams();
  if (params.page) q.set("page", String(params.page));
  if (params.pageSize) q.set("pageSize", String(params.pageSize));
  if (params.city) q.set("city", params.city);
  if (params.category) q.set("category", params.category);
  if (params.trustTier) q.set("trustTier", params.trustTier);
  if (params.search) q.set("search", params.search);
  const qs = q.toString();
  return publicFetch<ProListResponse>(
    `/pros${qs ? `?${qs}` : ""}`,
    120,
  );
};
